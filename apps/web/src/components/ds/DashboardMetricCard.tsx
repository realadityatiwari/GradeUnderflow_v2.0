...
export function DashboardMetricCard({
  ...
  return (
    <article
      className={cn(
        "group relative min-h-[172px] overflow-hidden rounded-2xl border border-white/[0.08] bg-zinc-950/65 p-5 shadow-[0_18px_45px_-28px_rgba(0,0,0,0.95)] transition-all duration-300 hover:-translate-y-1 hover:border-white/[0.16] hover:bg-zinc-900/80",
        className
      )}
    >
      ...
      <div className="relative flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-3 text-[2.25rem] font-semibold leading-none tracking-[-0.07em] text-zinc-50 tabular-nums">
            {value}
          </div>
        </div>
        <div
          className={cn(
            "grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-white/10 bg-gradient-to-br text-white shadow-lg",
            accent
          )}
        >
          {icon}
      ...
      <div className="relative mt-5">
        <div className="mb-2 flex items-center justify-between gap-3">
          ...
          {safeProgress !== null && (
            <span className="text-[11px] font-semibold tabular-nums text-zinc-300">{Math.round(safeProgress)}%</span>
          )}
        </div>
        {safeProgress !== null && (
